...
const buildConcordance = (annotationData) => {
  ...
  const words = Object.keys(dict).filter(x => x).sort(wordSort)
  let tmpl = ``
 ...
 */
const renderConcordance = (annotationData, form, manifest) => {
 ...
 */
export const initializeConcordance = async (manifest = { sequences: [] }) => {
  ...
  // Extract lines from all sequences
  const promises = []
  ...
  // Handle any deferred annotations
  for (const contents of await Promise.all(promises)) {
    if (contents && contents.length) {
      await extractLines({
        canvases: [{ otherContent: contents }]
      }, annotationData, manifest)
    }
  }
